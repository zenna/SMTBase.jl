...
isnonnegconstraint(c) = isa(c, GTEVar) && args(c)[2].val == 0

function string(c::ConstrainedParameter)
  # There are many nonnegative constraints so we print them specially as n⁺ rather than
  # n | n > 0, because otherwise it gets cumbersome to show them all.
  constraintstrings = []
  hasnonneg = false
  for constraint in c.constraints
    if isnonnegconstraint(constraint)
      hasnonneg = true
      continue
    end
    push!(constraintstrings, string(constraint))
  end
...
string(p::IfElseVar) =
  string("if ", string(p.args[1]), " then ", p.args[2], " else ", p.args[3])
